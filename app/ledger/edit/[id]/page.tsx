    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-900">내역 수정</h1>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                <button type="button" onClick={() => setCategoryId("")}
                  className={`py-2 text-xs rounded-xl border transition ${
                    categoryId === "" ? "bg-gray-400 text-white border-gray-400" : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"
                  }`}>
                  미선택
                </button>
          ...
          <div>
            ...
            {paymentMethods.length === 0 ? (
              ...
              <div className="grid grid-cols-3 gap-2">
                <button type="button" onClick={() => setPaymentMethodId("")}
                  className={`py-2 text-xs rounded-xl border transition ${
                    paymentMethodId === "" ? "bg-gray-400 text-white border-gray-400" : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"
                  }`}>
                  미선택
                </button>
          ...
          <div className="flex gap-3 pt-1">
            <Link href={`/ledger?year=${year}&month=${month}`}
              className="flex-1 py-3 bg-gray-100 text-gray-600 rounded-xl font-medium text-sm text-center hover:bg-gray-200 transition">
              취소
            </Link>
            <button type="submit" disabled={submitting}
              className="flex-1 py-3 bg-emerald-500 text-white rounded-xl font-medium text-sm hover:bg-emerald-600 transition disabled:opacity-50">
...
export default function EditTransactionPage() {
  return <Suspense><EditTransactionForm /></Suspense>;
}
